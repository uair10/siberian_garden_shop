...
from flask_admin.form import namegen_filename, thumbgen_filename
...
class FileUploadInput(object):
    ...
    def __call__(self, field, **kwargs):
        ...
        return Markup(
            template
            % {
                "text": html_params(type="text", readonly="readonly", value=value, name=field.name),
...
class MultipleFileUploadInput(object):
    ...
    data_template = "<div>" "    %(files)s" "</div>" "<input %(file)s multiple>"
    ...
    def __call__(self, field, **kwargs):
        ...
        return Markup(template % {"files": files, "file": html_params(type="file", **kwargs)})
...
class MultipleImageUploadInput(object):
    ...
    data_template = '<div class="image-thumbnail">' "    %(images)s" "</div>" "<input %(file)s multiple>"
    ...
    def __call__(self, field, **kwargs):
        ...
        if field.data and isinstance(field.data, string_types):
            ...
            args["images"] = "&emsp;".join(
                [
                    "<img src='{}' /><input type='checkbox' name='_{}-delete'>Delete</input>".format(src, filename)
                    for src, filename in attributes
                ]
            )
...
class FileUploadField(StringField):
    ...
    def process(self, formdata, data=unset_value, extra_filters=None):
        ...
        return super(FileUploadField, self).process(formdata, data, extra_filters)
...
class MultipleFileUploadField(FileUploadField):
    ...
    def process(self, formdata, data=unset_value, extra_filters=None):
        ...
        return super(MultipleFileUploadField, self).process(formdata, data, extra_filters)
...
class MultipleImageUploadField(ImageUploadField):
    ...
    def process(self, formdata, data=unset_value, extra_filters=None):
        ...
        return super(MultipleImageUploadField, self).process(formdata, data, extra_filters)
